...
def estimate_homographies(path, grid_size, square_size):
    ...
    return_value, corners = cv2.findChessboardCorners(
        image, patternSize=grid_size)
...
def compute_K(HH):
    ...
    for HI in HH:
        ...
        V = np.vstack(
            (V, np.hstack(vij_matrix(HI, 0, 0) - vij_matrix(HI, 1, 1))))
...
def compute_error(image):
    ...
    for i, point in enumerate(corners):
        ...
        error = error + ((P[0, :]@m)/(P[2, :]@m) - point[0]
                         )**2 + ((P[1, :]@m)/(P[2, :]@m) - point[1])**2
...
def draw_points(figure, image):
    ...
    for i, point in enumerate(corners):
        ...
        m = np.array([real_coordinates[i, 0], real_coordinates[i, 1], 0, 1])
        cv2.circle(figure, (round(point[0]), round(
            point[1])), 5, color=(0, 0, 255), thickness=1)
        cv2.circle(figure, (round((P[0, :]@m)/(P[2, :]@m)), round(
            (P[1, :]@m)/(P[2, :]@m))), 5, color=(255, 0, 0), thickness=1)
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    plt.savefig(f'{path}/{image["name"]} - points.png',
...
def superimpose_cylinder(figure, image, a, b, r, h):
    ...
    circle_inf = P @ np.vstack((positionsX, positionsY,
                               positionsZ, np.ones_like((positionsX))))
    if tz >= 0:
        circle_sup = P @ np.vstack((positionsX, positionsY,
                                   positionsZ + h, np.ones_like((positionsX))))
    else:
        circle_sup = P @ np.vstack((positionsX, positionsY,
                                   positionsZ - h, np.ones_like((positionsX))))
    ...
    cv2.polylines(figure, np.int32(
        [circle_inf.transpose()]), isClosed=True, color=(0, 0, 0), thickness=2)
    cv2.fillPoly(overlay, np.int32(
        [circle_inf.transpose()]), color=(255, 0, 0))
    ...
    cv2.polylines(figure, np.int32(
        [circle_sup.transpose()]), isClosed=True, color=(0, 0, 0), thickness=2)
    cv2.fillPoly(overlay, np.int32(
        [circle_sup.transpose()]), color=(0, 255, 0))
    ...
    for i in range(h):
        overlay = figure.copy()
        if tz >= 0:
            circle_int = P @ np.vstack((positionsX, positionsY,
                                       positionsZ + i, np.ones_like((positionsX))))
        else:
            circle_int = P @ np.vstack((positionsX, positionsY,
                                       positionsZ - i, np.ones_like((positionsX))))

        circle_int = (circle_int/circle_int[2])[:2]
        cv2.polylines(overlay, np.int32(
            [circle_int.transpose()]), isClosed=True, color=(128, 128, 128), thickness=1)
        figure = cv2.addWeighted(overlay, alpha, figure, 1 - alpha, 0)
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    plt.savefig(f'{path}/{image["name"]} - cylinder.png',
                bbox_inches='tight', pad_inches=0)
    plt.show()
